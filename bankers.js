let available = [];
  ...
  // Log initial matrices
  console.log("\n--- SYSTEM INITIALIZED ---");
  console.log("🟢 Available Resources:", available);
  console.log("📋 Max Matrix:");
  console.table(maxMatrix);
  console.log("🗂️ Allocation Matrix:");
  console.table(allocMatrix);
  console.log("📌 Need Matrix:");
  console.table(needMatrix);
}
  ...
  const parsedReq = req.split(',').map(Number);
  const result = handleRequest(parsedPid, parsedReq);

  const requestBox = document.getElementById("request-box");
  const safetyBox = document.getElementById("safety-box");

  console.log(`\n--- REQUEST SUBMISSION ---`);
  console.log(`Process ${parsedPid} requests: [${parsedReq.join(', ')}]`);
  ...
  if (result.error) {
    ...
    console.log(`🔐 Safe sequence: ${result.sequence.join(' → ')}`);
    console.log(`📦 Updated Available: [${available.join(', ')}]`);
    console.log(`🗂️ Updated Allocation Matrix:`);
    console.table(allocMatrix);
    console.log(`📋 Updated Need Matrix:`);
    console.table(needMatrix);
  }
}
